...
const sendRequest = async (
  ...
  const selfOutgoing: string[] =
    (await env.REQUESTS_OUT_KV.get(from, "json")) ?? [];
...
const acceptRequest = async (
  ...
  await env.REQUESTS_IN_KV.put(
    to,
    JSON.stringify(otherIncoming.filter((x) => x != from)),
  );
  await env.REQUESTS_OUT_KV.put(
    to,
    JSON.stringify(otherOutgoing.filter((x) => x != from)),
  );
  // Remove request from user
  await env.REQUESTS_IN_KV.put(
    from,
    JSON.stringify(selfIncoming.filter((x) => x != to)),
  );
  await env.REQUESTS_OUT_KV.put(
    from,
    JSON.stringify(selfOutgoing.filter((x) => x != to)),
  );
